...
var guesses = 0;
var isSunk = false;
var emptyinput = /(?<![\,\d])\d+(?![\,\d])/; //Переменные для проверки пустого или некорректного ввода
var emptyinput2 = /(?<![\.\d])\d+(?![\.\d])/;
...
while (isSunk == false) {
    ...
    } else if (emptyinput.test(guess) == false || emptyinput2.test(guess) == false) { //Проверка пустого или некорректного ввода
